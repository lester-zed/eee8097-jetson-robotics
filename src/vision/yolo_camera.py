        ...
        try:
            while monotonic() < deadline:
                ...
                if len(matching_detections) == 1:
                    ...
                    self.logger.warning(
                        "Multiple '%s' targets detected; "
                        "waiting for an unambiguous target.",
                        self.target_label,
                    )
                ...
                if self.show_preview:
                    ...
                    cv2.putText(
                        ...
                        (
                            ...
                            f"stable: {consecutive_detections}/"
                ...
                if (
                    latest_detection is not None
                    and consecutive_detections >= self.stable_frames
                ):
                    self.logger.info(
            ...
            self.logger.warning(
                "Target detection timed out after %.1f seconds.",
                timeout_seconds,
            )
            return None
